...
def submit_folders(folder_list, config_file=None, fixed_background=False):
    assert all(os.path.exists(folder) for folder in folder_list), str(folder_list)
    for folder in folder_list:

        if fixed_background:
            this_config_file = get_bg_config_file(folder)
        else:
...
if __name__ == '__main__':
    ...
    submit_folders(folder_list, config_file, fixed_background)

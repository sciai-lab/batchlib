...
def load_config(input_folder, output_folder, output_root_name, parser,
                gpu, n_cpus,
...
def run_all_workflows(input_folder, folder, gpu, n_cpus,
    ...
    if run_pixelwise:
        config = load_config(input_folder, folder, output_root_name, parse_pixel_config1, gpu, n_cpus,
    ...
    if run_instance1:
        ...
        config = load_config(input_folder, folder, output_root_name, parse_instance_config1, gpu, n_cpus,
                             use_unique_output_folder=use_unique_output_folder)
    ...
    if run_instance2:
        # TODO enable running with different analysis key?
        config = load_config(input_folder, folder, output_root_name, parse_instance_config2, gpu, n_cpus,
                             use_unique_output_folder=use_unique_output_folder)
...
# TODO allow to over-ride the default config files
if __name__ == '__main__':
    ...
    run_all_workflows(args.input_folder, args.folder, args.gpu, args.n_cpus)
